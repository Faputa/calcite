...
/**
 * @author zhangyin
 * @version 1.0.0 2024/5/9 下午4:01
 * @since 1.0.0
 */
public class TopDownRecRuleDriver implements RuleDriver {
  ...
  TopDownRecRuleDriver(VolcanoPlanner planner) {
  ...
  private void exploreMaterializationRoots() {
    for (RelSubset extraRoot : planner.explorationRoots) {
      ...
      for (RelNode rel : new ArrayList<>(extraRoot.set.rels)) {
        if (planner.isLogical(rel)) {
          new OptimizeMExpr(rel, extraRoot, true).perform();
        }
      }
    }
  }
  ...
  private void clearProcessed(RelSet set) {
    ...
    List<RelSubset> rootSubset = new ArrayList<>();
    for (RelSubset subset : set.subsets) {
      if (subset.resetTaskState() || explored) {
        ...
        if (subset == planner.root) {
          rootSubset.add(subset);
        }
      }
    }
    for (RelSubset subset : rootSubset) {
      new OptimizeGroup(subset, planner.infCost).perform();
    }
  }
  ...
  @Override public void onProduce(RelNode node, RelSubset subset) {
    ...
    if (!planner.isLogical(node)) {
      ...
      new OptimizeMExpr(node, applying.group(),
          applying.exploring() && !optimizing).perform();
    }
  }
  ...
  private class OptimizeGroup implements Task {
    ...
    @Override public void perform() {
      ...
      ArrayList<RelNode> rels = new ArrayList<>(group.set.rels);
      for (RelNode rel : rels) {
        if (!planner.isLogical(rel) && !rel.isEnforcer()) {
      ...
      for (RelNode rel : rels) {
        if (planner.isLogical(rel)) {
          new OptimizeMExpr(rel, group, false).perform();
        }
      }

      // A gate keeper to update context.
      new GroupOptimized(group).perform();
    }
  ...
  private class OptimizeMExpr implements Task {
    ...
    @Override public void perform() {
      ...
      // 1. explore input.
      // 2. apply other rules.
      int size = mExpr.getInputs().size();
      for (int i = size - 1; i >= 0; --i) {
        new ExploreInput(mExpr, i).perform();
      }
      new ApplyRules(mExpr, group, explore).perform();
    }
  ...
  private class EnsureGroupExplored implements Task {
    ...
    @Override public void perform() {
      if (parent.getInput(inputOrdinal) != input) {
        new ExploreInput(parent, inputOrdinal).perform();
        return;
      }
      input.setExplored();
  ...
  private class ExploreInput implements Task {
    ...
    @Override public void perform() {
      ...
      for (RelNode rel : new ArrayList<>(group.set.rels)) {
        if (planner.isLogical(rel)) {
          new OptimizeMExpr(rel, group, true).perform();
        }
      }
      new EnsureGroupExplored(group, parent, inputOrdinal).perform();
    }
  ...
  private class ApplyRules implements Task {
    ...
    @Override public void perform() {
      ...
      while (match != null) {
        ApplyRule applyRule = new ApplyRule(match, group, exploring);
  ...
  private @Nullable RelNode convert(RelNode rel, RelSubset group) {
    ...
    if (match != null) {
      new ApplyRule(match, group, false).perform();
    }
    return null;
  }
  ...
  private class OptimizeInput1 implements Task {
    ...
    @Override public void perform() {
      ...
      new OptimizeGroup(input, upperForInput).perform();
      new CheckInput(null, mExpr, input, 0, upperForInput).perform();

      // Apply enforcing rules.
      new DeriveTrait(mExpr, group).perform();
    }
  }
  ...
  private class OptimizeInputs implements Task {
    ...
    @Override public void perform() {
      ...
      while (processingChild < childCount) {
        ...
        ++ processingChild;
        new OptimizeGroup(input, upper).perform();
        new CheckInput(this, mExpr, input, processingChild - 1, upper).perform();
        if (processingChild < childCount) {
          this.perform();
        }
        break;
      }
  ...
  private class DeriveTrait implements GeneratorTask {
    ...
    @Override public void perform() {
      ...
      new ApplyRules(mExpr, group, false).perform();
    }
